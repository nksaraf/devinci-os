...
const throwTransferHandler: TransferHandler<ThrownValue, SerializedThrownValue> = {
  ...
  deserialize(serialized) {
    if (serialized.isError) {
      if (serialized.type === 'ApiError') {
        let error = new ApiError(serialized.value.code ?? ErrorCode.ENOTSUP);
        error.stack = serialized.value.stack;
        error.message = serialized.value.message;
        error.name = serialized.value.name;
        error.code = serialized.value.errno;
        throw error;
      }
