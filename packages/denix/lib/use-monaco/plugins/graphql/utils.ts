...
} from 'graphql-language-service-types.ts';

import type * as monaco from 'monaco-editor.ts';
...
export function toCompletion(
  ...
  return {
    ...
    insertText:
      entry.insertText || (!entry.label.startsWith('$') ? entry.label : entry.label.substring(1)),
