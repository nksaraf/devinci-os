import { MonacoWorker, IWorkerContext } from '../../worker.ts';
import { printSchema } from 'graphql.ts';

import type { FormattingOptions, ICreateData } from './types.ts';

import type { worker, editor, Position, IRange } from 'monaco-editor.ts';

import { getRange, LanguageService } from 'graphql-language-service.ts';

import type {
  SchemaResponse,
  CompletionItem as GraphQLCompletionItem,
} from 'graphql-language-service.ts';

import { toGraphQLPosition, toMonacoRange, toMarkerData, toCompletion } from './utils.ts';
import type * as monacoApi from 'monaco-editor.ts';
import type { GraphQLSchema, DocumentNode } from 'graphql.ts';
