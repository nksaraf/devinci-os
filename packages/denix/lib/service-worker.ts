...
// import TranspileWorker from './transpiler.worker.ts?worker.ts';
import { wrap } from './comlink/mod.ts';

import { newPromise } from './promise.ts';
import { ApiError, ERROR_KIND_TO_CODE } from './error.ts';
import { fromWireValue, toWireValue } from './comlink/http.handlers.ts';
...
export async function createLocalNetwork(processManager: ProcessManager) {
  const transpileHandler: ResponseResolver<RestRequest, RestContext, any> = async (
    req,
    res,
    ctx,
  ) => {
    ...
    let contentType = orig.headers.get('content-type') ?? 'application/javascript';
    let data;
    if (
  ...
  let worker = setupWorker(
    ...
    rest.post('/~fs*', async (req, res, ctx) => {
      ...
      if (fnName.endsWith('Sync')) {
        try {
          let result = await processManager.fs[
            fnName.substring(0, fnName.length - 4) as keyof typeof processManager.fs
          ](...argList);
  ...
  const transpiler = wrap<{ transpile(data: ArrayBuffer | ReadableStream): string }>(
    new Worker('./transpile.worker.ts?worker_file'),
  );
}
