  ...
  return {
    ...
    async load(id) {
      ...
      await new Promise(async (resolve, reject) => {
        const params = [
          opts.useAsBind
            ? [require.resolve('as-bind/lib/assembly/as-bind.ts'), '--exportRuntime']
            : [],
          id,
          '-b',
          wasmFilePath,
          ...Object.entries(opts.compilerOptions).map(([opt, val]: [string, any]) => {
            if (val === true) {
              return `--${opt}`;
            }
            return `--${opt}=${val}`;
        ...
        asCompiler.main(params, (err: Error) => {
          if (err) {
            reject(`${err}`);
            return -1;
          }
      ...
      return `
         export const wasmUrl = "wasm:${wasmFilePath}";
