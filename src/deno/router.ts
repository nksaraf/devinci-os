 ...
 * fails
 */
export type ErrorHandler = (req: Request, err: unknown) => Response | Promise<Response>;
 ...
 */
export function defaultUnknownMethodHandler(_req: Request, knownMethods: string[]): Response {
  return new Response(null, {
    status: 405,
    headers: {
        ...
        if (res !== null) {
          for (const [method, handler] of Object.entries(methods)) {
            if (req.method === method) {
              return await handler(req, res.pathname.groups);
            }
          }
          if (methods['any']) {
