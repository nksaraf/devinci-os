...
export default class IoDevicesDefault {
  ...
  constructor(wasmFs: WasmFs) {
    ...
    this.wasmFs.volume.mkdirSync(IO_DEVICES_CONSTANTS.FILE_PATH.DEVICE_FRAMEBUFFER_ZERO.PATH, {
      recursive: true,
    });
    ...
    const originalWindowSizeWrite = this.wasmFs.volume.fds[this.fdWindowSize].node.write;
    ...
    const originalBufferIndexDisplayWrite =
      this.wasmFs.volume.fds[this.fdBufferIndexDisplay].node.write;
