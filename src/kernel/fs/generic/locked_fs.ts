...
import type { IFileSystem, CallbackOneArg, CallbackTwoArgs } from '../core/file_system';
import type { ApiError } from '../core/api_error';
import type { FileFlagString } from '../core/file_flag';
import type { default as Stats } from '../core/stats';
 ...
 */
export default class LockedFS<T extends IFileSystem> implements IFileSystem {
  private _fs: T;
  private _mu: Mutex;
  ...
  public readFileSync(fname: string, encoding: string, flag: FileFlagString): any {
    if (this._mu.isLocked()) {
      throw new Error('invalid sync call');
    }
