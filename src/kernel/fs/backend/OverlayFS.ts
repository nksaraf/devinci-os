 ...
 * @hidden
 */
function getFlag(f: string): FileFlagString {
  return FileFlagString.getFileFlag(f);
}
 ...
 */
export class UnlockedOverlayFS extends BaseFileSystem implements IFileSystem {
  public static isAvailable(): boolean {
    return true;
  }

  private _writable: IFileSystem;
  private _readable: IFileSystem;
  ...
  constructor(writable: IFileSystem, readable: IFileSystem) {
    super();
  ...
  public getOverlayedFileSystems(): { readable: IFileSystem; writable: IFileSystem } {
    return {
          ...
          case ActionType.NOP:
            return this._writable.exists(p, (exists: boolean) => {
              if (exists) {
                ...
                stats.mode = mode;
                this._readable.readFile(
                  p,
                  null,
                  getFlag('r'),
                  (readFileErr: ApiError, data?: any) => {
                    if (readFileErr) {
    ...
    this.stat(p, false, (err: ApiError, dirStats?: Stats) => {
      ...
      this._writable.readdir(p, (err: ApiError, wFiles: string[]) => {
        ...
        this._readable.readdir(p, (err: ApiError, rFiles: string[]) => {
          ...
          const filtered: string[] = wFiles
            .concat(rFiles.filter((fPath: string) => !this._deletedFiles[`${p}/${fPath}`]))
            .filter((fPath: string) => {
              // Remove duplicates.
  ...
  private updateLog(addition: string) {
    ...
    if (this._deleteLogUpdatePending) {
      ...
      this._writable.writeFile(deletionLogPath, this._deleteLog, 'utf8', 'w', 0o644, (e) => {
...
export interface OverlayFSOptions {
  ...
  writable: IFileSystem;
  // The file system that initially populates this file system.
  readable: IFileSystem;
}
