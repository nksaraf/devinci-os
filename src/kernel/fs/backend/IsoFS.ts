      ...
      if (spEntry instanceof SPEntry && spEntry.checkBytesPass()) {
        ...
        for (let i = 1; i < suEntries.length; i++) {
          ...
          if (
            ...
            (entry instanceof EREntry && entry.extensionIdentifier() === rockRidgeIdentifier)
          ) {
 ...
 */
export default class IsoFS extends SynchronousFileSystem implements IFileSystem {
  ...
  private constructor(data: Buffer, name: string = '') {
    ...
    candidateVDs.forEach((v) => {
      ...
      if (
        !this._pvd ||
        this._pvd.type() !== VolumeDescriptorTypeCode.SupplementaryVolumeDescriptor
      ) {
        this._pvd = v;
      }
    });
  ...
  public openSync(p: string, flags: FileFlagString, mode: number): File {
