 ...
 * ```
 */
export default class AsyncMirror extends SynchronousFileSystem implements IFileSystem {
  ...
  public static readonly Options: FileSystemOptions = {
    sync: {
      ...
      validator: (v: any, cb: CallbackOneArg) => {
  ...
  private _sync: IFileSystem;
  private _async: IFileSystem;
   ...
   */
  constructor(sync: IFileSystem, async: IFileSystem) {
    super();
    this._sync = sync;
    this._async = async;
  }
  ...
  public openSync(p: string, flag: FileFlagString, mode: number): File {
    ...
    return new MirrorFile(
      ...
      this._sync.readFileSync(p, null, 'r'),
    );
  }
   ...
   */
  private _initialize(userCb: CallbackOneArg): void {
    ...
    if (!this._isInitialized) {
      ...
      if (callbacks.push(userCb) === 1) {
        const copyDirectory = (p: string, mode: number, cb: CallbackOneArg) => {
            if (p !== '/') {
              this._sync.mkdirSync(p, mode);
            }
          ...
          copyFile = (p: string, mode: number, cb: CallbackOneArg) => {
            this._async.readFile(p, null, 'r', (err, data) => {
              if (err) {
                cb(err);
              } else {
                try {
                  this._sync.writeFileSync(p, data!, null, 'w', mode);
                } catch (e) {
                  err = e;
                } finally {
                  cb(err);
          ...
          copyItem = (p: string, cb: CallbackOneArg) => {
