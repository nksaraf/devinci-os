...
class FileDescriptorArgumentConverter {
  ...
  public toRemoteArg(
    ...
    p: string,
    flag: FileFlagString,
    cb: CallbackTwoArgs<IFileDescriptorArgument>,
  ): void {
    const id = this._nextId++;
      ...
      if (err) {
        ...
        if (isReadable(flag)) {
          fd.read(
            ...
            stats!.size,
            0,
            (err?: ApiError | null, bytesRead?: number, buff?: Buffer) => {
              if (err) {
                cb(err);
              } else {
 ...
 */
export default class WorkerFS extends BaseFileSystem implements IFileSystem {
    ...
    worker.addEventListener('message', (e: MessageEvent) => {
      ...
      if (isAPIRequest(request)) {
        ...
        switch (request.method) {
          ...
          case 'probe':
            (() => {
              const rootFs = <IFileSystem>fs.getRootFS(),
