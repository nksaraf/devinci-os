...
export default class HTML5FS extends BaseFileSystem implements IFileSystem {
   ...
   */
  public static Create(opts: HTML5FSOptions, cb: CallbackTwoArgs<HTML5FS>): void {
  ...
  public open(p: string, flags: FileFlagString, mode: number, cb: CallbackTwoArgs<IFile>): void {
    ...
    this.fs.root.getFile(
      p,
      {
        create: pathNotExistsAction(flags) === ActionType.CREATE_FILE,
        exclusive: isExclusive(flags),
      },
   ...
   */
  private _readdir(path: string, cb: CallbackTwoArgs<Entry[]>): void {
    const error = (err: DOMException): void => {
      cb(convertError(err, path, true));
    };
   ...
   * returned
   */
  private _remove(path: string, cb: CallbackOneArg, isFile: boolean): void {
