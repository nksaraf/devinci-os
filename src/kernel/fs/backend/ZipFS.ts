...
import { FileFlagString, ActionType, pathExistsAction, isWriteable } from '../core/file_flag';
...
export class CentralDirectory {
  ...
  constructor(private zipData: Buffer, private data: Buffer) {
    ...
    if (this.data.readUInt32LE(0) !== 0x02014b50) {
      throw new ApiError(
        ErrorCode.EINVAL,
        `Invalid Zip file: Central directory record has invalid signature: ${this.data.readUInt32LE(
          0,
        )}`,
      );
    }
...
export class EndOfCentralDirectory {
  constructor(private data: Buffer) {
    if (this.data.readUInt32LE(0) !== 0x06054b50) {
      throw new ApiError(
        ErrorCode.EINVAL,
        `Invalid Zip file: End of central directory record has invalid signature: ${this.data.readUInt32LE(
          0,
        )}`,
      );
    }
 ...
 */
export default class ZipFS extends SynchronousFileSystem implements IFileSystem {
