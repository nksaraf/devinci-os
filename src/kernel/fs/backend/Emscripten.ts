...
export default class EmscriptenFileSystem extends SynchronousFileSystem {
  ...
  public openSync(p: string, flag: FileFlagString, mode: number): EmscriptenFile {
    try {
      const stream = this._FS.open(p, getFlagString(flag), mode);
  ...
  public readFileSync(p: string, encoding: string, flag: FileFlagString): any {
    try {
      const data: Uint8Array = this._FS.readFile(p, { flags: getFlagString(flag) });
  ...
  public writeFileSync(
    ...
    try {
      ...
      this._FS.writeFile(p, u8, { flags: getFlagString(flag), encoding: 'binary' });
