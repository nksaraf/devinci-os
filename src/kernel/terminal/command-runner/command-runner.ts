...
export default class CommandRunner {
  ...
  async runCommand() {
    ...
    try {
      ...
      if (this.tty) {
        this.tty.print('\r\n');
        this.tty.print(`shell: parse error (${c.toString()})\r\n`);
      }
      console.error(c);
  ...
  _addStdinToSharedStdin(data: Uint8Array, processObjectIndex: number) {
    ...
    const sharedStdin = this.spawnedProcessObjects[processObjectIndex].sharedStdin;
    let startingIndex = 1;
    if (sharedStdin[0] > 0) {
  ...
  async _spawnProcess(commandOptionIndex: number) {
    ...
    spawnedProcessObject.process.start(
      this.pipedStdinDataForNextProcess.length > 0 ? this.pipedStdinDataForNextProcess : undefined,
    );
    ...
    if (this.commandOptionsForProcessesToRun.length > commandOptionIndex + 1) {
      isNextCallbackCommand =
        this.commandOptionsForProcessesToRun[commandOptionIndex + 1].callback !== undefined;
    }
  ...
  async _spawnProcessAsWorker(commandOptionIndex: number) {
    ...
    const workerBlobUrl = await this._getBlobUrlForProcessWorker(processWorkerUrl, this.tty);
  ...
  _processDataCallback(
    ...
    if (commandOptionIndex < this.commandOptionsForProcessesToRun.length - 1) {
      ...
      if (
        ...
        this.spawnedProcessObjects[this.spawnedProcessObjects.length - 1].commandOptionIndex >
          commandOptionIndex
      ) {
  ...
  async _getBlobUrlForProcessWorker(processWorkerUrl: string, wasmTty?: WasmTty) {
  ...
  async _getCommandOptionsFromAST(
    ...
    const envEntries = Object.entries(ast.env).map(([key, value]: [string, any]) => [
      key,
      value.value,
    ]);
    let env: any = {};
