 ...
 * @hidden
 */
function assertRoot(fs?: IFileSystem | null): IFileSystem {
  if (fs) {
    return fs;
  }
  throw new ApiError(
 ...
 * TODO
 * INCOMPLETE: Should aim to have this match the types of require('fs');
 * This layer handles:
 *
...
export default class NodeFileSystem {
  ...
  private root: IFileSystem | null = null;
  private fdMap: { [fd: number]: File } = {};
  private nextFd = 100;
  events: any;

  public initialize(rootFS: IFileSystem): IFileSystem {
   ...
   */
  public getRootFS(): IFileSystem | null {
    if (this.root) {
      return this.root;
    } else {
      return null;
  ...
  public open(
    ...
    try {
      assertRoot(this.root).open(
        normalizePath(path),
        FileFlagString.getFileFlag(flag),
        mode,
        (e: ApiError, file?: File) => {
          if (file) {
  ...
  public readFile(filename: string, arg2: any = {}, cb: CallbackTwoArgs<any> = nopCb) {
    ...
    const newCb = wrapCb(cb, 2);
    try {
      const flag = FileFlagString.getFileFlag(options['flag']);
      if (!isReadable(flag)) {
        return newCb(
  ...
  public appendFile(filename: string, data: any, arg3?: any, cb: CallbackOneArg = nopCb): void {
    ...
    try {
      ...
      if (!isAppendable(flag)) {
        return newCb(
          new ApiError(ErrorCode.EINVAL, 'Flag passed to appendFile must allow for appending.'),
        );
      }
  ...
  public watchFile(
    filename: string,
    arg2: any,
    listener: (curr: Stats, prev: Stats) => void = nopCb,
  ): void {
  ...
  public watch(
    filename: string,
    arg2: any,
    listener: (event: string, filename: string) => any = nopCb,
  ): _fs.FSWatcher {
    throw new ApiError(ErrorCode.ENOTSUP);
  }
...
export interface FSModule extends NodeFileSystem {
  /**
   * The FS constructor.
   */
  FS: typeof NodeFileSystem;
  /**
   ...
   */
  getFSModule(): NodeFileSystem;
  /**
   * Set the FS object backing the fs module.
   */
  changeFSModule(newFs: NodeFileSystem): void;
}
